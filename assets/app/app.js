...
var app = {
	init: () => {
		...
		var announceCheck = $(
			"<label><input type='checkbox' id='isHideMaxPlayers'/> <span>Hide row/s with 5 players?</span></label>"
		);
	...
	getClubSettingsCallback: res => {
		settings.firstTeeTime = moment(res.club.opening_time, "HH:mm").format(
			"hh:mm A"
		);
		settings.lastTeeTime = moment(res.club.closing_time, "HH:mm").format(
			"hh:mm A"
		);
	...
	setCourses: res => {
		for (var i = 0; i < res.courses.length; i++) {
			$("#coursesSelect").append(
				"<option value=" +
					res.courses[i].facility_id +
					">" +
					res.courses[i].facility +
					"</option>"
			);
		}
	...
	renderFlights: () => {
		...
		for (var i = 0; i < flightsF.length; i++) {
			...
			var teetimeRow =
				app.splitCourse(flightsF[i].course) == "Front 9"
					? $(".teetime_" + xTime + ".F-9")
					: $(".teetime_" + xTime + ".B-9");
			if (flightsF[i].course == "Front 9") {
				for (var x = 0; x < flightMateCount; x++) {
					teetimeRow
						.find(".flight-player-" + (x + 1) + "-teetime_" + xTime)
						.html(flightsF[i].flightmates[x].golfer);
				}
			} else {
				for (var x = 0; x < flightMateCount; x++) {
					teetimeRow
		...
		var createBody = data => {
			...
			do {
				...
				for (var i = 1; i <= 2; i++) {
					...
					row.append(
						"<td class='flight-player-td flight-player-5-teetime_" +
							startTime +
							"'></td>"
					);
	...
	stopPagingTeeTimeTable: () => {
		...
		tableRows.removeClass(
			"paging-batch-1 paging-batch-2 paging-batch-3 paging-batch-4 hide"
		);
