let fids = {
	...
	init: () => {
		...
		var announceCheck = $(
			"<label><input type='checkbox' id='isAnnounce'/> <span>First to Announce?</span></label>"
		);
	...
	getClubSettingsCallback: res => {
		// fids.settings.firstTeeTime = moment(res.club.opening_time, "HH:mm").format(
		// 	"hh:mm A"
		// );
		// fids.settings.lastTeeTime = moment(res.club.closing_time, "HH:mm").format(
		// 	"hh:mm A"
		// );
	...
	setCourse: () => {
		$.each(fids.settings.courses, function(i, v) {
			$("#coursesSelect").append(
				"<option value=" + v.facility_id + ">" + v.facility + "</option>"
			);
		});
	...
	setUpFidsTable: () => {
		...
		var startTime = moment(fids.settings.firstTeeTime, "HH:mm A")
			.subtract(30, "m")
			.format("x");
		...
		do {
			...
			for (var i = 1; i <= 2; i++) {
				var row = $(
					"<tr class='teetime_" +
						...
						(!isHidden ? "hide" : "") +
						"' data-time=" +
						moment(startTime, "x").format("HH:mm") +
						"></tr>"
				);
				row.append(
				...
				row.append(
					"<td class='flight-course'>" +
						(i == 1 ? "Front 9" : "Back 9") +
						"</td>"
				);
				if (counter <= 2) {
					row.append(
						"<td class='flight-player-td td-player-1 initial-flight flight-player-1-teetime_" +
							startTime +
							"'>-</td>"
					);
					row.append(
						"<td class='flight-player-td td-player-2 initial-flight flight-player-2-teetime_" +
							startTime +
							"'>-</td>"
					);
					row.append(
						"<td class='flight-player-td td-player-3 initial-flight flight-player-3-teetime_" +
							startTime +
							"'>-</td>"
					);
					row.append(
						"<td class='flight-player-td td-player-4 initial-flight flight-player-4-teetime_" +
							startTime +
							"'>-</td>"
					);
					row.append(
						"<td class='flight-player-td td-player-5 initial-flight flight-player-5-teetime_" +
							startTime +
							"'>-</td>"
					);
				} else {
					row.append(
						"<td class='flight-player-td td-player-1 flight-player-1-teetime_" +
							startTime +
							"'></td>"
					);
					row.append(
						"<td class='flight-player-td td-player-2 flight-player-2-teetime_" +
							startTime +
							"'></td>"
					);
					row.append(
						"<td class='flight-player-td td-player-3 flight-player-3-teetime_" +
							startTime +
							"'></td>"
					);
					row.append(
						"<td class='flight-player-td td-player-4 flight-player-4-teetime_" +
							startTime +
							"'></td>"
					);
					row.append(
						"<td class='flight-player-td td-player-5 flight-player-5-teetime_" +
							startTime +
							"'></td>"
					);
				}
			...
			startTime = moment(startTime, "x").add(
				fids.settings.teeTimeInterval,
				"m"
			);
			counter++;
	...
	getFlights: c_id => {
		var options = {};
		options.url = "http://192.168.1.44:5006/api/teetimes";
		options.data = { date: "2019-02-15" };
		...
		fids.flights = [
			{
				...
				teetime: 6.666666,
				p1: "Rica Shoal",
				p2: "JUAN PABLO RODRIGUEZ",
				p3: "Rene D. Enriquez",
				p4: "MAX J. CARTER",
				p5: "LINDA CARTER BUSH",
				active: true
			},
			{
				id: 5,
	...
	renderFlights: flights => {
		...
		$.each(flights, function(i, v) {
			console.log(convertPrecision(v.teetime));
			var teetimeX = moment(convertPrecision(v.teetime), "HH:mm").format("x");
			var row = tBody.find(
				"tr.teetime_" + teetimeX + "." + convertCourseType(v.course)
			);

			row.find("td.td-player-1").append(v.p1);
			row.find("td.td-player-2").append(v.p2);
			row.find("td.td-player-3").append(v.p3);
			row.find("td.td-player-4").append(v.p4);
			row.find("td.td-player-5").append(v.p5);
	...
	rowMonitoring: () => {
		var rows = fids.fidsTable
			.find("tbody")
			.children()
			.not(".hide"); //fids.rowCheck()
	...
	generateTextToSpeechAnnouncement: (
		player1,
		player2,
		player3,
		player4,
		...
		var message =
			...
			teetime +
			" teetime, the following players must proceed to the teebox now. ";
		if (player1) message += player1 + ", ";
		if (player2) message += player2 + ", ";
		if (player3) message += player3 + ", ";
		if (player4) message += player4 + ", ";
		if (player5) message += player5 + ", ";

		message +=
			". You will Tee Off in " +
			(fids.checkIfFirstToAnnounce() ? 29 : 20 - 2) +
			" minutes at " +
			course +
			" course. Thank You!";
		return message;
	...
	announcement: () => {
		// Edit for sample purposes
		var currTime = moment()
			.add(fids.checkIfFirstToAnnounce() ? 20 : 20 - 2, "m")
			.format("hh:mm A");
		var currTimeX = moment(currTime, "hh:mm A").format("x");
		var rows = fids.fidsTable
			.find("tbody")
		...
		if (fids.announced.indexOf("first") == -1) {
			...
			fids.announced.includes("first") &&
			fids.announced.indexOf("second") == -1
		) {
			re = 7;
		}
		...
		if (isAnnounce) {
			if (row.find("td.td-player-1").text() != "") {
				var course =
					$("#headerTitle").text() + " " + row.find("td.flight-course").text();
				...
				fids.mssg = fids.generateTextToSpeechAnnouncement(
					player1,
					player2,
					player3,
					player4,
	...
	announcementV2: () => {
		...
		if (
			moment(rowTime, "HH:mm").isSame(moment(tNow, "HH:mm")) ||
			moment(rowTime, "HH:mm").isSame(moment(firstTeeTime, "HH:mm")) ||
			moment(rowTime, "HH:mm").isSame(moment(secondTeeTime, "HH:mm")) ||
			moment(rowTime, "HH:mm").isSame(moment(thirdTeeTime, "HH:mm"))
		) {
			fids.announcementCounter = fids.announcementCounter + 1;
		...
		if (isAnnounce) {
			if (row.find("td.td-player-1").text() != "") {
				...
				var course =
					$("#headerTitle").text() + " " + o.find("td.flight-course").text();
	...
	hideCheck: () => {
		...
		var firstTeeTime = "02:00",
			...
			thirdTeeTime = "02:20",
			fourthTeeTime = "02:30";
		var fTteeTime = moment(firstTeeTime, "HH:mm")
			.add(1, "m")
			.format("HH:mm");
		var sTteeTime = moment(secondTeeTime, "HH:mm")
			.add(1, "m")
			.format("HH:mm");
		var tTteeTime = moment(thirdTeeTime, "HH:mm")
			.add(1, "m")
			.format("HH:mm");
		var h = false;
	...
	rowCheck: () => {
		...
		if (moment(currTime, "HH:mm").isSame(moment(firstTeeTime, "HH:mm"))) {
			...
			moment(currTime, "HH:mm").isSame(moment(secondTeeTime, "HH:mm"))
		) {
			rows = fids.fidsTable
				.find("tbody")
				.children()
			...
			moment(currTime, "HH:mm").isSame(moment(thirdTeeTime, "HH:mm"))
		) {
			rows = fids.fidsTable
				.find("tbody")
				.children()
...
var convertPrecision = function(val) {
	const mins = val * 60;
